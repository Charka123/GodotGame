...
func _run() -> void:
	...
	await delivery.deliver_package(&"block", Vector2i.ZERO)
	assert(economy.beans == 75)
	assert(board.occupied_cells[Vector2i(2, 0)].blocks_packages)
	...
	assert(endpoints.back() == Vector2i(2, 0))
	await delivery.deliver_package(&"production", Vector2i(2, 0))
	assert(endpoints.back() == Vector2i(2, 0), "Starting on a blocker resolves there")
	...
	print("PASS: controller delivery, delayed single payout, gaps, Block transit/cost/stopping, invalid targets, unsupported packages, insufficient funds")
	quit()
